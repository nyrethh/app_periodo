...
const LogModal = ({ day, data, onClose, onSave, theme, onAddSymptom }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50 p-4"><div className="bg-white rounded-3xl shadow-2xl p-6 w-full max-w-md transform transition-all scale-95 animate-modal-pop-in"><div className="flex justify-between items-center mb-4"><h2 className={`text-2xl font-bold ${theme.modalHeader} font-sans`}>{formattedDate}</h2><button onClick={onClose} className="text-gray-400 hover:text-pink-500 transition-colors"><X size={28} /></button></div><div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div><h3 className={`text-lg font-semibold ${theme.secondaryText} mb-3 flex items-center`}>Síntomas</h3><div className="grid grid-cols-3 gap-3">{INITIAL_SYMPTOMS.map(symptom => (<button key={symptom.id} onClick={() => setSelectedSymptoms(prev => prev.includes(symptom.id) ? prev.filter(id => id !== symptom.id) : [...prev, symptom.id])} className={`p-3 rounded-xl text-center transition-all duration-200 border-2 ${selectedSymptoms.includes(symptom.id) ? 'bg-yellow-200 border-yellow-400 scale-105' : 'bg-gray-100 border-transparent'}`}><span className="text-2xl">{symptom.icon}</span><p className="text-xs font-medium text-gray-700 mt-1">{symptom.label}</p></button>))}</div>{showMoreSymptoms && <div className="grid grid-cols-3 gap-3 mt-3 animate-modal-pop-in">{MORE_SYMPTOMS.map(symptom => (<button key={symptom.id} onClick={() => setSelectedSymptoms(prev => prev.includes(symptom.id) ? prev.filter(id => id !== symptom.id) : [...prev, symptom.id])} className={`p-3 rounded-xl text-center transition-all duration-200 border-2 ${selectedSymptoms.includes(symptom.id) ? 'bg-yellow-200 border-yellow-400 scale-105' : 'bg-gray-100 border-transparent'}`}><span className="text-2xl">{symptom.icon}</span><p className="text-xs font-medium text-gray-700 mt-1">{symptom.label}</p></button>))}</div>}<button onClick={() => setShowMoreSymptoms(!showMoreSymptoms)} className="w-full mt-3 text-purple-600 font-semibold">{showMoreSymptoms ? 'Mostrar menos' : 'Más síntomas...'}</button></div>
